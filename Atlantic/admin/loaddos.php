<?php
    ...
    $columns = ['id', 'username', 'username_password', 'tipo'];
    ...
    if ($num_rows > 0) {
        while ($row = $resultado->fetch_assoc()) {
            ...
            $output['data'] .= '<td>' . $row['username_password'] . '</td>';
            $output['data'] .= '<td>' . $row['tipo'] . '</td>';
            $output['data'] .= '<td><a class="btn btn-warning btn-sm" href="editar.php?id=' . $row['id'] . '">Editar</a></td>';
            $output['data'] .= "<td><a class='btn btn-danger btn-sm' href='eliminardos.php?id=" . $row['id'] . "'>Eliminar</a></td>";
            $output['data'] .= '</tr>';
        }
    } else {
        $output['data'] .= '<tr>';
